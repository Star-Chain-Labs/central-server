...
const PROVIDER_BASE = process.env.CLIENT_API_URL;
console.log("🌐 Provider Base URL:", PROVIDER_BASE);
    ...
    for (const sport of sports) {
      try {
        ...
        if (competitions.length === 0) {
          console.warn(`⚠️ [Competitions] ${sport.name}: 0 from provider`);
          continue;
        }
        ...
        await Competition.bulkWrite(ops);
        console.log(
          `✅ [Competitions] ${sport.name}: ${competitions.length} synced`,
        );
      } catch (innerErr) {
        console.error(`❌ [Competitions] ${sport.name}:`, innerErr.message);
      }
  ...
  try {
    ...
    for (const sport of sports) {
      try {
        ...
        if (events.length === 0) {
          console.warn(`⚠️ [Events] ${sport.name}: 0 from provider`);
          continue;
        }
        ...
        await Event.bulkWrite(ops);
        console.log(`✅ [Events] ${sport.name}: ${events.length} synced`);
      } catch (innerErr) {
        console.error(`❌ [Events] ${sport.name}:`, innerErr.message);
      }
    }
...
export const syncMarkets = async () => {
  try {
    ...
    if (events.length === 0) {
      console.warn(`⚠️ [syncMarkets] No events in DB`);
      return;
    }
    ...
    console.log(
      `✅ [Markets] ${successCount} events, ${totalMarkets} markets (${failCount} failed)`,
    );
  } catch (err) {
    console.error("❌ [Markets Sync]", err.message);
  }
};
